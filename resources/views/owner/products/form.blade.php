    ...
    <div class="max-w-3xl">
        <div class="bg-white rounded-2xl border border-slate-200/60 p-6 sm:p-8">
            ...
            <form method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div x-data="sizeManager()">
                    <label class="block text-xs font-medium text-slate-500 mb-3">Ukuran & Harga</label>
                    <p class="text-[11px] text-slate-400 mb-3">Atur variasi ukuran produk dengan harga tambahan. Harga akhir = Harga Dasar + Harga Tambahan</p>
                    
                    <div class="space-y-3">
                        <template x-for="(size, index) in sizes" :key="index">
                            <div class="grid grid-cols-12 gap-3 items-end p-3 bg-slate-50 rounded-xl">
                                <div class="col-span-4">
                                    <label class="block text-[10px] font-medium text-slate-500 mb-1">Nama Ukuran</label>
                                    <input type="text" :name="'sizes['+index+'][name]'" x-model="size.name"
                                        placeholder="e.g., Normal (15cm)" required
                                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                                </div>
                                <div class="col-span-3">
                                    <label class="block text-[10px] font-medium text-slate-500 mb-1">Harga Tambahan</label>
                                    <input type="number" :name="'sizes['+index+'][additional_price]'" x-model="size.additional_price"
                                        placeholder="0" min="0" required
                                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                                </div>
                                <div class="col-span-3">
                                    <label class="block text-[10px] font-medium text-slate-500 mb-1">Stok</label>
                                    <input type="number" :name="'sizes['+index+'][stock]'" x-model="size.stock"
                                        placeholder="0" min="0" required
                                        class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                                </div>
                                <div class="col-span-2">
                                    <button type="button" @click="removeSize(index)"
                                        class="w-full px-3 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 text-xs font-medium transition">
                                        Hapus
                                    </button>
                                </div>
                            </div>
                    ...
                    <button type="button" @click="addSize()"
                        class="mt-3 px-4 py-2 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 text-sm font-medium transition">
                        + Tambah Ukuran
                    </button>
                </div>
                ...
                @php
                    $defaultSizes = [
                        ['name' => 'Normal (15cm)', 'additional_price' => 0, 'stock' => 0],
                        ['name' => 'Regular (20cm)', 'additional_price' => 50000, 'stock' => 0],
                        ['name' => 'Large (25cm)', 'additional_price' => 100000, 'stock' => 0],
                    ];
                ...
                function sizeManager() {
                    return {
                        sizes: @json($sizes),
                        addSize() {
                            this.sizes.push({ name: '', additional_price: 0, stock: 0 });
                        },
                        removeSize(index) {
