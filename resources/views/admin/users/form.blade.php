...
@section('title', ($user->exists ? 'Edit' : 'Tambah') . ' User — Admin Aqlaya Cake')
@section('page-title', $user->exists ? 'Edit User' : 'Tambah User')

@section('content')
    {{-- Back link --}}
    <a href="{{ route('admin.users.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-800 transition mb-6 group">
    ...
    <div class="max-w-xl">
        <div class="bg-white rounded-2xl border border-slate-200/60 p-6 sm:p-8">
            {{-- Header --}}
            <div class="mb-6">
                <h2 class="text-xl font-semibold text-slate-900">{{ $user->exists ? 'Perbarui Akun User' : 'Akun User Baru' }}</h2>
                <p class="text-sm text-slate-500 mt-1">{{ $user->exists ? 'Perbarui informasi profil atau ganti password akun user' : 'Buat akun user baru untuk customer' }}</p>
            </div>
            ...
            <form method="POST" action="{{ $formAction }}" class="space-y-5">
                ...
                <div class="flex items-center gap-3 pt-4 border-t border-slate-100">
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 transition">
                        ...
                        {{ $user->exists ? 'Simpan Perubahan' : 'Buat Akun User' }}
                    </button>
                    <a href="{{ route('admin.users.index') }}" class="px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition">
                        Batal
                    </a>
                </div>
            </form>
